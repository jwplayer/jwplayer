...
(function(jwplayer, undefined) {
	...
	var api = jwplayer.api = function(container) {
		...
		// given a name getBuffer, it adds to *this* a function which internally triggers jwGetBuffer
		function generateInternalFunction(index, name) {
			var internalName = 'jw' + name.charAt(0).toUpperCase() + name.slice(1);

			_this[name] = function(arg1) {
				if (arg1 !== undefined) {
		...
		_this.setInstream = function(instream) {
			// TODO: Shouldn't these be chainable?
			_instream = instream;
			return instream;
		};
	...
	jwplayer.api.destroyPlayer = function(playerId) {
		...
		var toDestroy = document.getElementById(player.id + (player.renderingMode == "flash" ? "_wrapper" : ""));
		if (toDestroy) {

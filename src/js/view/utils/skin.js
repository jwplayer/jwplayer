...
export function handleColorOverrides(playerId, skin) {
    ...
    function styleControlbar(config) {
        ...
        if (config.icons) {
            ...
            css(
                `#${playerId} .jw-icon-cast button.jw-off`,
        ...
        if (config.iconsActive) {
            ...
            // Can't use addStyle since it will camel case the style name
            css(
                `#${playerId} .jw-icon-cast:hover button.jw-off`,
                `{--disconnected-color: ${config.iconsActive}}`,
                playerId
            );
            css(
                `#${playerId} .jw-icon-cast:focus button.jw-off`,
                `{--disconnected-color: ${config.iconsActive}}`,
                playerId
            );
            css(
                `#${playerId} .jw-icon-cast button.jw-off:focus`,
                `{--disconnected-color: ${config.iconsActive}}`,
                playerId
            );

            css(
                `#${playerId} .jw-icon-cast button`,
                `{--connected-color: ${config.iconsActive}}`,
                playerId
            );
            css(
                `#${playerId} .jw-icon-cast button:focus`,
                `{--connected-color: ${config.iconsActive}}`,
                playerId
            );
            css(
                `#${playerId} .jw-icon-cast:hover button`,
                `{--connected-color: ${config.iconsActive}}`,
                playerId
            );
            css(
                `#${playerId} .jw-icon-cast:focus button`,
                `{--connected-color: ${config.iconsActive}}`,
                playerId
            );
        }
    ...
    function styleTimeslider(config) {
        ...
        addStyle(
            ['.jw-buffer'],
            'background-color',
            getRgba(config.progress, 50)
        );

        addStyle(['.jw-rail'], 'background-color', config.rail);

        addStyle(
            ['.jw-background-color.jw-slider-time', '.jw-slider-time .jw-cue'],
            'background-color',
            config.background
        );
    }
    ...
    function insertGlobalColorClasses(config) {
        if (config.textActive) {
            ...
            css(
                `#${playerId} .jw-color-active-hover:hover`,
                activeColorSet,
                playerId
            );
        }
        if (config.text) {
            ...
            css(
                `#${playerId} .jw-color-inactive-hover:hover`,
                inactiveColorSet,
                playerId
            );
        }

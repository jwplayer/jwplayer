...
export function embed(swfUrl, container, id, wmode) {
    let swf;
    let queueCommands = true;
    ...
    if (Browser.msie) {
        ...
        const objectElements = container.getElementsByTagName('object');
        for (let i = objectElements.length; i--;) {
    ...
    addGetter(swf, 'trigger', function(type, json) {
        ...
        processEventsTimeout = setTimeout(function() {
            let length = eventQueue.length;
            processEventsTimeout = -1;
            while (length--) {
                const event = eventQueue.shift();
                if (event.json) {
                    const data = JSON.parse(decodeURIComponent(event.json));
    ...
    addGetter(swf, 'triggerFlash', function(name) {
        ...
        if (name !== 'setup' && queueCommands || !swf.__externalCall) {
            const commandQueue = swf.__commandQueue;
            // remove any earlier commands with the same name
            for (let j = commandQueue.length; j--;) {

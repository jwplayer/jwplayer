...
describe('Mobile Behavior', function() {
    ...
    function setContainerWidth(width) {
        model.set('containerWidth', width);
    }

    it('should pause when menu is open on breakpoint 0', function() {
        setContainerWidth(250); // breakpoint 0
        api.play();
    ...
    it('should pause when menu is open on breakpoint 1', function() {
        setContainerWidth(320); // breakpoint 1
        api.play();
        expect(api.playing).to.equal('playing');
        settingsMenu.open();
        expect(api.playing).to.equal('paused');
    ...
    it('should not pause when menu is open on breakpoints higher than 1', function() {
        setContainerWidth(420); // breakpoint 2
        api.play();
        expect(api.playing).to.equal('playing');
        settingsMenu.open();
        expect(api.playing).to.equal('playing');
    ...
    describe('LIVE streams', function() {
        beforeEach(() => {
            model.set('streamType', 'LIVE');
        });

        it('should not pause when menu is open on breakpoint 0', function() {
            setContainerWidth(250); // breakpoint 0
            api.play();
            expect(api.playing).to.equal('playing');
            settingsMenu.open();
        ...
        it('should not pause when menu is open on breakpoint 1', function() {
            setContainerWidth(320); // breakpoint 1
            api.play();
            expect(api.playing).to.equal('playing');
            settingsMenu.open();

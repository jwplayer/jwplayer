define([
    ...
    describe('provider primary priority', function() {
        ...
        it('no primary defined', function() {
            ...
            assert.isOk(providerList.length >= 3, 'There are at least 3 providers listed');

            var keys = getKeyIndexes(providerList);
            assert.isOk(keys.html5 < keys.flash, 'HTML5 has higher priority than flash');
            assert.isOk(keys.html5 > keys.youtube, 'HTML5 has lower priority than youtube');
            assert.isOk(keys.flash > keys.youtube, 'Flash has lower priority than youtube');
        });
        ...
        it('flash primary requested', function() {
            ...
            var keys = getKeyIndexes(providerList);
            assert.isOk(keys.html5 > keys.flash, 'HTML5 has lower priority than flash');
            assert.isOk(keys.html5 > keys.youtube, 'HTML5 has lower priority than youtube');
            assert.isOk(keys.flash > keys.youtube, 'Flash has lower priority than youtube');
        });

	...
	html5.controlbar = function(api, config) {
		...
		function _init() {
			...
			(new html5.skinloader(config.skin, function(skin) {
				...
				_layout = _api.skin.controlbar.layout ? _api.skin.controlbar.layout : _defaults.layout;
				_createStyles();
				_buildControlbar();
		...
		function _insertCSSRule(rule) {
			_styleSheet.insertRule(rule, _styleSheet.cssRules.length);
		}
		
		function _createStyles() {
	  		_insertCSSRule('.controlbar{position:absolute;overflow: hidden}');
	  		_insertCSSRule('.controlbar span{float:left;height:100%}');
	  	    
	  	    _insertCSSRule('.group{position:relative;display:inline}');
	  	    _insertCSSRule('.left{float:left}');
			_insertCSSRule('.right {position:relative;float:right!important}');
	  	    _insertCSSRule('.center{position:absolute!important}');
	  	    
	  	    _insertCSSRule('.controlbar button{position:relative;display:inline-block;height:100%;border:none;cursor:pointer;float:left}');
	  	    
	  	    _insertCSSRule('.slider, .rail span {position:absolute;height:100%;width:100%}');
		}
		

		function _buildControlbar() {
			_controlbar = document.createElement("span");
			_controlbar.id = _id;
			_controlbar.className = "controlbar";
			_css(_controlbar, {
		...
		function _buildElement(element) {
			switch (element.type) {
			case CB_DIVIDER:
				return _buildImage(element.name);
				break;
			case CB_TEXT:
		...
		function _buildImage(name, style, stretch) {
		...
		function _buildButton(name) {
			...
			element.id = _id + "_" + name + Math.round(Math.random()*10000000); 
			...
			_insertCSSRule('#' + element.id + ' { background: url('+ outSkin.src +') center no-repeat}');
			if (overSkin.src) {
				_insertCSSRule('#' + element.id + ':hover { background: url('+ overSkin.src +') center no-repeat}');
			}
		...
		function _buildSlider(name) {
			...
			for (var i=0; i<railElements.length; i++) {
				var element = _buildImage(name + "Slider" + railElements[i], {width: "100%", backgroundRepeat: "repeat-x"}, true);
				if (element) {
		...
		function _styleTimeSlider(slider) {
			_css(_elements['timeSliderCapRight'], { right: 0, position: JW_CSS_ABSOLUTE });
		...
		var _setProgress = this.setProgress = function(pct) {
			...
			var progress = _elements['timeSliderProgress'];
			var thumb = _elements['timeSliderThumb'];
			
			if (Math.abs(_lastProgress-pct) > 0.05) {
				if (progress) { 
					(new html5.utils.animations(progress, 'width', _lastProgress * 100 + "%", pct * 100 + "%", 1000, html5.utils.animations.easing.linear.easeIn)).start();
				} 
				if (thumb) { 
					(new html5.utils.animations(thumb, 'left', _lastProgress * 100 + "%", pct * 100 + "%", 1000, html5.utils.animations.easing.linear.easeIn)).start();
				}
			} else {
			   if (progress) progress.style.width = 100 * pct + "%";
			   if (thumb) thumb.style.left = pct * _utils.parseDimension(_elements['timeSliderRail'].clientWidth) + "px"; 
			}
			
			if (thumb) {
				if (pct <= 0 || pct >= 1 && thumb.style.opacity == 1) {
					html5.utils.animations.fadeOut(thumb);
				} else if (thumb.style.opacity == 0) {
					html5.utils.animations.fadeIn(thumb);
				}
			}
